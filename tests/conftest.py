...
@pytest.fixture(scope="module")
def surfWalletSingleton(SurfWallet, entryPoint, owner):
    """
    Fetch GnosisSafe Singleton Contract from the specified address
    """
    return SurfWallet.deploy(entryPoint.address, {'from': owner})
...
@pytest.fixture(scope="module")
def surfWalletProxy(SurfWalletProxy, owner, surfWalletSingleton):
    """
    Deploy a proxy contract for GnosisSafe
    """
    contract = SurfWalletProxy.deploy(surfWalletSingleton.address,
                                      {'from': owner})
    # returning a proxy instance with the target abi to facilitate diligate call
    cwp = Contract.from_abi(
        "SurfWallet", contract.address, surfWalletSingleton.abi)
...
def depositPaymaster(DepositPaymaster, entryPoint, TokenPriceOracle, tokenErc20, owner):
    ...
    paymaster.addToken(tokenErc20.address, uo.address)
    return paymaster
